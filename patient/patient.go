...
func pickRace() string {
	races, ok := (world.Demographics["Race"]).([]utils.Choice)
	if !ok {
...
func pickEthnicity(race string) string {
	ethMap, ok := (world.Demographics["Ethnicity"]).(map[string][]utils.Choice)
	if !ok {
...
func pickBloodType(race string) string {
	bloodMap, ok := (world.Demographics["BloodType"]).(map[string][]utils.Choice)
	if !ok {
